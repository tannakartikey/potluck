...
-- contribute_note: a TRUSTED contributor (trust_level >= 1) submits a task AND its already-produced
-- note in one call — for sources only they can process (e.g. a long video transcript the sealed,
-- no-web worker can't fetch, and which won't fit a task prompt). The task lands 'done' with the
-- attributed, unverified result; the producer is the donor of record. Untrusted contributors must use
-- submit_task instead (a direct write-a-note path is gated to keep it from becoming a spam vector).
