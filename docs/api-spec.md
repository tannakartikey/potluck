...
**Budget / denial-of-wallet — enforced entirely client-side.** The central API treats `subtasks.token_budget` as advisory. The Runner is the sole enforcement point: an advisory token-budget skip (refuse a task declaring more than the local cap), **no tools** (so no agentic tool-loop), and a 5-minute wall-clock timeout. The server can never specify an unbounded task; the contributor's machine holds the cap. (A hard per-run $ cap, duplicate-call debounce, and output-size cap are TODO.) `token_count` reported here is for provenance, not a server-side limit.

**Text-only safe mode (security invariant).** The Runner invokes the agent with **all tools explicitly denied** (`--tools "" --strict-mcp-config --disallowed-tools "<every built-in>"`; an empty `--allowedTools` is a no-op and is NOT used) inside a locked-down container — no shell, no file I/O, no web fetch, no MCP. The agent cannot take consequential actions regardless of what the (untrusted) `prompt` says. A client-side pre-publish output guard scans the artifact for secret patterns / local paths (not content/policy) and sets `p_output_guard_passed`. `submit_result` **requires `output_guard_passed = true`** (it raises otherwise), so a jailbroken task cannot push a leaking artifact into the public commons.
